        ...
        if (isset($_POST['recover'])) {
            ...
            if ($count > 0) {
                ...
                <form action = "" method = "POST">
                    Security question:<input type='text' name='question1'>
                    <button type='submit' name='reset'>Submit</button>
                </form>
                <?php
                $req = $db->prepare("SELECT securityquestion, securityanswer FROM member WHERE userName =:username");
            $req->bindParam(":username", $login_username);
            $req->execute();
            $count = $req->rowCount();
            $data = $req->fetchall();
            foreach ($data as $row) {
...
//    $password = password_hash($_POST['password'], PASSWORD_DEFAULT); (thanks!!! very useful- sp)
